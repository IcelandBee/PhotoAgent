...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Photography Viewpoint Agent: depth + point-cloud rendering")
    ...
    parser.add_argument('--viewpoint-backend', choices=['depth_3d'], default='depth_3d')
    parser.add_argument('--viewpoint-horizontal-fov-deg', type=float, default=60)
    parser.add_argument('--viewpoint-border-mode', choices=['constant'], default='constant')
    parser.add_argument('--depth-model',help='HF model id or local directory; default selected by depth backend')
    parser.add_argument('--depth-backend',choices=['auto','depth_anything','depth_pro','precomputed'],default='auto')
    parser.add_argument('--depth-metadata-path',help='DepthObservation JSON sidecar; default depth-path with .json suffix')
    parser.add_argument('--depth-path',help='Precomputed positive Z depth .npy matching selected reference frame')
