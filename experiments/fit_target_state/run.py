...
from experiments.fit_target_state.detection import ExperimentDetector
...
def run_fit(reference_path,gt_path,output_dir,config,ignore_path=None,detector=None,progress=print,reference_ignore_path=None):
    ...
    reference_ignore = ignore_mask(reference.size,reference_ignore_path,[]) if reference_ignore_path else None
    ...
    if config.subject_mode == 'reposition':
        detector = detector or ExperimentDetector(config)
        ...
        if config.use_manual_gt_bbox:
            gt_box = config.gt_subject_bbox
            detection_log['ground_truth'] = {'source':'manual_override','bbox':gt_box}
        else:
    ...
    prepared = PreparedRenderer(reference,observation,preview_size,reference_ignore)
    ...
    exclusion |= prepared.transformed_ignore(target,preview_size)
    ...
    log = {
        ...
        "inputs":{"reference":str(Path(reference_path).resolve()),"ground_truth":str(Path(gt_path).resolve()),
                  ...
                  "reference_ignore_mask":str(Path(reference_ignore_path).resolve()) if reference_ignore_path else None},
        ...
        "loss_definition":{
            ...
            "background":"(1-background_color_weight)*(0.7*grayscale_L1 + 0.3*Sobel_magnitude_L1) + background_color_weight*RGB_L1 on valid background",
            "global":"grayscale_L1 on GT-valid, non-ignored pixels (persons included)",
            "mask":"GT person bbox + rendered person bbox + projected original person bbox + GT ignore mask/rects + transformed reference ignore mask + GT letterbox; 3px preview safety margin",
...
def main(argv=None):
    ...
    parser.add_argument('--reference-ignore-mask',type=Path)
    ...
    try:
        ...
        log = run_fit(args.reference,args.ground_truth,args.output_dir,config,args.ignore_mask,
                      reference_ignore_path=args.reference_ignore_mask)
